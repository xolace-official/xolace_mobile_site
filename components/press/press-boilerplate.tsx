...
function CopyButton({ text, label }: { text: string; label: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="flex items-center gap-1.5 text-[10px] font-mono uppercase tracking-[0.14em] text-muted-foreground/40 hover:text-foreground transition-colors duration-200"
  ...
  return (
    <section className="section-spacing bg-xo-surface-lowest overflow-hidden relative">
      ...
      <div aria-hidden className="absolute inset-0 pointer-events-none">
        <div className="absolute w-[550px] h-[550px] rounded-full bg-primary/[0.05] blur-[140px] -top-20 -right-10 animate-gentle-pulse" />
        <div className="absolute w-[400px] h-[400px] rounded-full bg-accent/[0.04] blur-[120px] bottom-0 left-1/4 animate-gentle-pulse [animation-delay:3s]" />
      </div>
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            ...
            <p className="text-sm text-muted-foreground/40 font-light pb-1 max-w-xs sm:text-right">
              Use these directly. Email us first if you&apos;d like to describe Xolace differently.
            </p>
          </div>
        </motion.div>
        ...
        <div className="space-y-5">
          {descriptions.map(({ key, label, sublabel, text }, i) => (
            <motion.div
              ...
              className="rounded-2xl border border-border/20 bg-background/40 p-8 space-y-6"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <p className="text-sm font-medium text-foreground/80">{label}</p>
                  <span className="text-[10px] font-mono uppercase tracking-[0.14em] text-muted-foreground/30 border border-border/30 rounded-full px-2 py-0.5">
                    {sublabel}
                  </span>
                </div>
              ...
              <p className="font-serif text-base md:text-lg text-foreground/70 leading-relaxed whitespace-pre-line">
                {text}
              </p>
            </motion.div>
          ))}
